...
<div class="d-flex align-items-center justify-content-between mb-4">
    <h2 class="fw-bold mb-0"><i class="bi bi-house text-primary me-2"></i>Welcome to BarangayHelpDesk</h2>
    <span class="text-muted small"><?= $total ?> announcement(s)</span>
</div>

<?php if (!isLoggedIn()): ?>
<div class="bhd-card p-4 mb-4 bg-primary bg-opacity-10 border-0">
    <div class="d-flex align-items-center gap-3 flex-wrap">
        <div class="flex-grow-1">
            <h5 class="fw-bold mb-1">Have a concern for the barangay?</h5>
            <p class="mb-0 text-muted">Login or register to submit and track your concerns online.</p>
        </div>
        <div class="d-flex gap-2">
            <a href="<?= BASE_URL ?>/resident/login.php" class="btn btn-primary">Login</a>
            <a href="<?= BASE_URL ?>/resident/register.php" class="btn btn-outline-primary">Register</a>
        </div>
    </div>
</div>
<?php endif; ?>

<h5 class="fw-semibold mb-3"><i class="bi bi-megaphone text-warning me-2"></i>Announcements</h5>
            ...
            <div class="bhd-card p-4 announcement-card <?= $ann['is_pinned'] ? 'pinned' : '' ?>">
                <div class="d-flex align-items-start gap-3">
                    ...
                    <div class="flex-grow-1">
                        ...
                        <?php else: ?>
                            <p class="text-muted mb-2"><?= nl2br(sanitize(substr($ann['content'], 0, 200))) ?>... <a href="<?= BASE_URL ?>/resident/login.php">Login to read more</a></p>
                        <?php endif; ?>
                        <small class="text-muted">
                            <i class="bi bi-person me-1"></i><?= sanitize($ann['author']) ?>
                            &bull;
                            <i class="bi bi-calendar me-1"></i><?= formatDate($ann['published_at']) ?>
                        </small>
                    </div>
                </div>
            </div>
